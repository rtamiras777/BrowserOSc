...
export class BrowserAgent {
  ...
  private _getSpecialTaskMetadata(task: string): {task: string, metadata: ExecutionMetadata} | null {
    ...
    // Ciroos.ai Navigation Task
    if (taskLower === "read about our vision and upvote ❤️" || taskLower === "read about our vision") {
      return {
        task: "Navigate to Ciroos.ai",
        metadata: {
          executionMode: 'predefined' as const,
          predefinedPlan: {
            agentId: 'ciroos-navigator',
            name: "Ciroos Navigator",
            goal: "Navigate to Ciroos.ai website",
            steps: [
              "Navigate to https://ciroos.ai/"
            ]
          }
        }
      };
    ...
    // AWS Integration Task
    if (taskLower === "integrate me into aws" || taskLower === "integrate into aws") {
      return {
        task: "Complete AWS Integration",
        metadata: {
          executionMode: 'direct' as const,
          predefinedPlan: {
            agentId: 'aws-integration-assistant',
            name: "AWS Integration Assistant",
            goal: "Complete AWS integration by creating CloudFormation stack and auto-filling Ciroos form with stack outputs",
            steps: [
              "Use navigate tool to go to https://ravi-dev.ciroos.ai/settings/integrations.",
              "Use visual_click on AWS Connect button.",
              "Use human_input: 'Step 1: Enter a Role Name (e.g. CiroosRole) and check EKS permissions if needed. Click Done when ready.'",
              "Use visual_click on Launch Stack button.",
              "Use wait tool for 2 seconds.",
              "Use human_input: 'AWS CloudFormation page is now open in a new tab. Please sign in to AWS if prompted, then click Done to continue.'",
              "Use human_input: 'Step 2: On CloudFormation Quick Create Stack page: 1) Upload the CloudFormation template you downloaded. 2) Enter Stack name (e.g. CiroosStack). 3) Scroll down and check the IAM acknowledgment checkbox. 4) Click Create Stack button. 5) Wait for CREATE_COMPLETE status (1-2 min). Click Done when complete.'",
              "Use extract tool to get RoleArn value from CloudFormation Outputs tab.",
              "Use tab_focus to return to Ciroos tab.",
              "Use human_input: 'Final step: Enter Configuration Name (e.g. Production AWS) then click Done.'",
              "Use visual_click on Next button.",
              "Use visual_click on Submit button."
            ]
          }
        }
      };
    ...
    // View Investigations Task
    if (taskLower === "view investigations") {
      return {
        task: "View Ciroos Investigations",
        metadata: {
          executionMode: 'predefined' as const,
          predefinedPlan: {
            agentId: 'view-investigations',
            name: "View Investigations",
            goal: "Navigate to Ciroos investigations page, change time range to last quarter, and expand investigation details",
            steps: [
              "Navigate to https://ravi-dev.ciroos.ai/incidents",
              "If login page is shown, click 'Continue with SSO' button",
              "If SSO popup appears, use human_input to ask user to complete SSO authentication, then automatically continue when page redirects back",
              "Click the time range dropdown selector at the top of the page to open the time range menu",
              "Click 'Last Quarter' option in the dropdown menu to filter investigations",
              "Wait 2 seconds for page to load filtered results",
              "Click on the first investigation row in the table to expand its details"
            ]
          }
        }
      };
  ...
  async execute(task: string, metadata?: ExecutionMetadata): Promise<void> {
    ...
    try {
      ...
      if (_metadata?.executionMode === 'direct' && _metadata.predefinedPlan) {
        // Direct execution: parse tool calls from steps and execute without LLM planning
        await this._executeDirect(_task, _metadata.predefinedPlan);
      } else if (_metadata?.executionMode === 'predefined' && _metadata.predefinedPlan) {
        // Predefined: use LLM planner with TODO list
        await this._executePredefined(_task, _metadata.predefinedPlan);
      } else {
        // Dynamic: full LLM planning
  ...
  /**
   * Execute plan with direct tool parsing (no LLM planning)
   * Parses tool calls from step descriptions and executes them sequentially
   */
  private async _executeDirect(task: string, plan: any): Promise<void> {
    this.executionContext.setCurrentTask(task);

    // Publish start message
    this._publishMessage(
      `Executing workflow: ${plan.name || 'Direct Execution'}`,
      "thinking"
    );

    Logging.log("BrowserAgent", `Starting direct execution with ${plan.steps.length} steps`, "info");
    ...
    for (let i = 0; i < plan.steps.length; i++) {
      ...
      Logging.log("BrowserAgent", `Direct step ${i + 1}/${plan.steps.length}: ${step}`, "info");

      // Parse tool calls from step description
      const toolCalls = this._parseToolCalls(step);

      if (toolCalls.length === 0) {
        // No parseable tool calls - fallback to executor with LLM
        Logging.log("BrowserAgent", `No tool calls parsed from step, using LLM executor`, "info");
        await this._runExecutor([step], { proposedActions: [step] } as any);
        continue;
      }
      ...
      for (const toolCall of toolCalls) {
        try {
          await this._executeToolDirectly(toolCall);
        } catch (error) {
          const errorMsg = error instanceof Error ? error.message : String(error);
          Logging.log("BrowserAgent", `Tool execution failed: ${errorMsg}`, "error");

          // On error, fallback to LLM-assisted execution for remaining steps
          this._publishMessage(`⚠️ Step failed, using AI assistant for remaining steps...`, "thinking");
          const remainingSteps = plan.steps.slice(i);
          const todoMarkdown = remainingSteps.map((s: string) => `- [ ] ${s}`).join('\n');
          this.executionContext.setTodoList(todoMarkdown);
          await this._executePredefined(task, { ...plan, steps: remainingSteps });
          return;
        }
      }
    }
    ...
    Logging.log("BrowserAgent", `Direct execution complete`, "info");
  }

  /**
   * Parse tool calls from a step description
   * Example: "Use navigate tool to go to https://example.com" -> [{tool: "navigate", args: {url: "https://example.com"}}]
   */
  private _parseToolCalls(step: string): Array<{tool: string, args: Record<string, any>}> {
    const toolCalls: Array<{tool: string, args: Record<string, any>}> = [];

    // Pattern: "Use <tool_name> to <action>"
    const toolPatterns = [
      // Navigate tool
      { regex: /use navigate tool to go to (https?:\/\/[^\s]+)/i, tool: 'navigate', extractArgs: (match: RegExpMatchArray) => ({ url: match[1] }) },

      // Visual click tool
      { regex: /use visual_click (?:to click|on) (?:the )?['""]?([^'"".]+?)['""]?(?:\s+button|\s+link)?/i, tool: 'visual_click', extractArgs: (match: RegExpMatchArray) => ({ description: match[1].trim() }) },

      // Visual type tool
      { regex: /use visual_type to (?:type|enter|fill) ['""]?([^'"".]+?)['""]? (?:in|into) (?:the )?['""]?([^'"".]+?)['""]?/i, tool: 'visual_type', extractArgs: (match: RegExpMatchArray) => ({ text: match[1].trim(), description: match[2].trim() }) },

      // Wait tool
      { regex: /use wait tool for (\d+) seconds?/i, tool: 'wait', extractArgs: (match: RegExpMatchArray) => ({ seconds: parseInt(match[1]) }) },

      // Human input tool
      { regex: /use human_input:? ['""]?(.+?)['""]?$/i, tool: 'human_input', extractArgs: (match: RegExpMatchArray) => ({ prompt: match[1].trim() }) },

      // Extract tool
      { regex: /use extract tool to get (.+?)(?:\.|$)/i, tool: 'extract', extractArgs: (match: RegExpMatchArray) => ({ instruction: match[1].trim() }) },

      // Scroll tool
      { regex: /use scroll tool to (find|locate) (.+?)(?:\.|$)/i, tool: 'scroll', extractArgs: (match: RegExpMatchArray) => ({ direction: 'down', amount: 500 }) },

      // Tab focus tool
      { regex: /use tab_focus to return to (.+?) tab/i, tool: 'tab_focus', extractArgs: (match: RegExpMatchArray) => ({ description: match[1].trim() }) },
    ];
  ...
  /**
   * Execute a single tool call directly without LLM
   */
  private async _executeToolDirectly(toolCall: {tool: string, args: Record<string, any>}): Promise<void> {
    const { tool: toolName, args } = toolCall;

    Logging.log("BrowserAgent", `Executing tool: ${toolName} with args: ${JSON.stringify(args)}`, "info");

    // Publish tool call as thinking message (using thinking type for visibility)
    this._publishMessage(
      `🔧 Calling ${toolName} with: ${JSON.stringify(args)}`,
      "thinking"
    );
    ...
    // Publish tool result as thinking message
    const resultPreview = typeof parsedResult === 'object'
      ? JSON.stringify(parsedResult, null, 2).slice(0, 200)
      : String(parsedResult).slice(0, 200);
    this._publishMessage(
      `✓ ${toolName} completed: ${resultPreview}`,
      "thinking"
    );

    // Check if tool failed
    if (parsedResult && typeof parsedResult === 'object' && 'ok' in parsedResult && !parsedResult.ok) {
      throw new Error(parsedResult.error || 'Tool execution failed');
    }

    Logging.log("BrowserAgent", `Tool ${toolName} executed successfully`, "info");
  }
