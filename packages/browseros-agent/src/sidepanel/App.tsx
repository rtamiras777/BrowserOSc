...
export function App() {
  ...
  // Load Gemini Vision API key from environment
  useEffect(() => {
    const apiKey = process.env.GEMINI_VISION_API_KEY
    if (apiKey) {
      chrome.storage.local.set({
        'gemini-vision-api-key': apiKey
      }, () => {
        console.log('✅ Gemini Vision API key loaded from environment')
      })
    }
  }, [])
    ...
    >
      <div className="h-screen bg-background overflow-x-hidden flex flex-col" role="main" aria-label="BrowserOS Chat Assistant">
        ...
        <Header
          ...
          showReset={messages.length > 0 || (appMode === 'teach' && teachModeState !== 'idle') || appMode === 'vision'}
        ...
        <div className="flex-1 min-h-0 overflow-hidden">
          {appMode === 'teach' ? (
            ...
            <VisionMode isConnected={connected} />
          ) : (
            <Chat
