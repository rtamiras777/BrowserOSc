...
export function ChatInput({ isConnected, isProcessing }: ChatInputProps) {
  ...
  const submitTask = (query: string) => {
    ...
    sendMessage(MessageType.EXECUTE_QUERY, {
      ...
      tabIds,
      source: 'sidepanel',
      chatMode: false  // Include chat mode setting (always false now)
    })
    ...
    // Clear selected tabs in agent mode
    clearSelectedTabs()
  ...
  const getPlaceholder = () => {
    ...
    return 'Tell me what action you want to take...'
  }

  const getHintText = () => {
    if (isProcessing) return 'Task running… Press Esc to cancel'
    return 'Agent mode is for complex web navigation tasks • Press Enter to send'
  }
